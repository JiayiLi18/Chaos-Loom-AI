Shader "Custom/ToonGradientRampShader"
{
    Properties
    {
        _MaskTexture("Mask Texture", 2D) = "black" {}
        [MainTexture] _MainTex("Albedo (RGB)", 2D) = "white" {}
        [RampTexture] _RampTex ("Ramp Texture", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(Varyings IN) : SV_Target
            {
                ...
                NdotL = NdotL * 0.5 + 0.5; // 关将[-1,1]映射到[0,1]
               ...
               // 阴影采样逻辑重构
                ...
                // 双重阴影混合算法
                float combinedShadow = saturate(shadowAtten + (1.0 - mainLight.shadowAttenuation));
                shadowAtten = smoothstep(0.0, 1.0, shadowAtten); 
                // 分层次采样Ramp纹理
                float2 baseRampUV = float2(NdotL, 0.0);
                float2 shadowRampUV = float2(shadowAtten, 0.0);
                
                // 动态混合采样结果
                float4 rampColor = SAMPLE_TEXTURE2D(_RampTex, sampler_RampTex, baseRampUV);
                float4 shadowRamp = SAMPLE_TEXTURE2D(_RampTex, sampler_RampTex, shadowRampUV);
                
                // 最终颜色合成
                finalColor.rgb = lerp(
                    finalColor.rgb * shadowRamp.rgb,
                    finalColor.rgb * rampColor.rgb,
                    smoothstep(0.3, 0.7, combinedShadow)
                ) * mainLight.color;
