    ...
    public static class ColorTextureGenerator
    {
        ...
        public static Texture2D GetOrCreateColorTexture(Color32 color)
        {
            ...
            if (File.Exists(texturePath))
            {
                ...
                {
                    // 确保纹理名称正确（防止从文件加载时名称丢失）
                    string expectedName = GetColorTextureName(normalizedColor);
                    if (string.IsNullOrEmpty(existingTexture.name) || existingTexture.name != expectedName)
                    {
                        existingTexture.name = expectedName;
                    }
