...
namespace Voxels
{
    ...
    public class VoxelJsonDB : MonoBehaviour
    {
        ...
        /// <summary>
        /// 同步VoxelDefinition到数据库（更新所有面纹理信息）
        /// </summary>
        public void SyncVoxelToDatabase(ushort typeId, VoxelDefinition def)
        {
            if (_cache == null || !_isInitialized)
            {
                Debug.LogError("[VoxelJsonDB] Database not initialized!");
            ...
            try
            {
                ...
                if (entry == null)
                {
                    Debug.LogError($"[VoxelJsonDB] Voxel with ID {typeId} not found in database!");
                    return;
                }
                ...
                // 同步所有面的纹理名称到数据库（仅当有有效新纹理时才覆盖）
                for (int i = 0; i < 6; i++)
                {
                    var face = def.faceTextures[i];
                    var tex = face != null ? face.texture : null;
                    if (tex != null && !string.IsNullOrEmpty(tex.name))
                    {
                        entry.face_textures[i] = tex.name + ".png";
                    }
                    else
                    {
                        // 没有新纹理：保持原值；若原值为 null 则设为空字符串
                        if (entry.face_textures[i] == null)
                            entry.face_textures[i] = string.Empty;
                    }
                }
                
                // 同步其他属性
                entry.name = def.displayName ?? def.name;
                entry.description = def.description;
                
                Debug.Log($"[VoxelJsonDB] Synced voxel {entry.name} (ID: {typeId}) to database");
            }
            catch (Exception ex)
            {
                Debug.LogError($"[VoxelJsonDB] Failed to sync voxel {typeId} to database: {ex.Message}");
            }
        }
